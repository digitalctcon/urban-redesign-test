...
def load_model():
    ...
    pipe = StableDiffusionInpaintPipeline.from_pretrained(
        "stable-diffusion-v1-5/stable-diffusion-inpainting",  # Updated model path
        torch_dtype=dtype
    ).to(device)
    return pipe
...
# ---- Main App UI ----
def log_generation(input_path, output_path, prompt):
    ...
    with open(log_file, mode='a', newline='') as file:
        ...
        if not file_exists:
            writer.writerow(["Input File", "Output File", "Prompt"])
        writer.writerow([input_path, output_path, prompt])
...
def main():
    ...
    if st.button("✨ Generate New Design") and "mask" in st.session_state:
        ...
        if "selected_image" in st.session_state:
            ...
            log_generation(original_image_path, new_image_path, prompt)  # Log the generation
        else:
            new_image_path = "urban_redesign.png"
            st.session_state["output_image"].save(new_image_path)
            log_generation("uploaded_image", new_image_path, prompt)  # Log the generation
